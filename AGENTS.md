## 1. このファイルの目的
このファイル（`AGENT.md`）は、本プロジェクト（FEVER CAPITAL）における AI エージェントおよび開発メンバー向けの共通運用ルールの「ハブ（ポータル）」として機能します。

プロジェクトを円滑に進めるための具体的なルールは、すべて `Docs/` ディレクトリ配下に階層的に整理されています。本ファイルには全体像とルールの優先順位のみを記載し、詳細は各ドキュメントを参照してください。
...
- **ルートディレクトリ**
  ...
  - `AGENT.md` (本ファイル): 当インデックスとルールの優先順位。
